...
<a href="{{ route('admin.popular_routes') }}" class="flex items-center px-4 py-3 {{ request()->routeIs('admin.popular_routes') ? 'bg-primary-maroon text-white font-bold' : 'text-gray-300 hover:text-white hover:bg-white/10 transition font-medium' }} rounded-xl mb-2">
    <i data-lucide="navigation" class="w-5 h-5 mr-3"></i> Popular Routes
</a>
...
<a href="{{ route(auth()->user()->getRolePrefix().'.reviews') }}" class="flex items-center px-4 py-3 {{ request()->routeIs(auth()->user()->getRolePrefix().'.reviews') ? 'bg-dark-maroon text-white font-bold' : 'text-gray-300 hover:bg-dark-maroon hover:text-white' }} rounded-xl transition-all mb-2">
    <i data-lucide="star" class="w-5 h-5 mr-3"></i> Manage Reviews
</a>

<a href="{{ route(auth()->user()->getRolePrefix().'.contact_messages') }}" class="flex items-center px-4 py-3 {{ request()->routeIs(auth()->user()->getRolePrefix().'.contact_messages') ? 'bg-dark-maroon text-white font-bold' : 'text-gray-300 hover:bg-dark-maroon hover:text-white' }} rounded-xl transition-all mb-2">
